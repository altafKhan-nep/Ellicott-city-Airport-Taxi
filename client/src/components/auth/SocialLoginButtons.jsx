...
// Passport OAuth redirect flow. Buttons render only when the provider is
// configured (mirrors server-side strategy registration / 503 guard).
...
export default function SocialLoginButtons() {
  if (!googleReady && !facebookReady) return null;
      ...
      <div className="mt-4 space-y-3">
        {googleReady && (
          <a
            ...
            className="flex w-full items-center justify-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-ink transition-colors hover:bg-slate-50"
          >
        ...
        {facebookReady && (
          <a
            href={`${API_ROOT}/api/auth/facebook`}
            className="flex w-full items-center justify-center gap-2 rounded-full border border-slate-300 bg-white px-4 py-3 text-sm font-semibold text-ink transition-colors hover:bg-slate-50"
          >
